...
def is_valid_url(url: str) -> bool:
    """Valida que la URL sea pública y use esquemas permitidos para evitar SSRF."""
    try:
        parsed = urlparse(url)
        ...
        # Bloquear IPs privadas comunes (RFC 1918 y otros)
        import ipaddress
        import socket
        try:
            # Extraer host (si tiene puerto, removerlo)
            host = domain.split(":")[0]
            ip = socket.gethostbyname(host)
            addr = ipaddress.ip_address(ip)
            return not (addr.is_private or addr.is_loopback or addr.is_unspecified)
        except:
            # Si no resuelve, permitimos que Playwright lo intente (fallará de forma segura)
            # pero bloqueamos si parece una IP privada directa
            pass
...
def get_mercadolibre_data(url):
    if not is_valid_url(url):
        logger.warning(f"[SCRAPER] URL no válida o bloqueada por seguridad: {url}")
        return {"name": "Error: URL no permitida", "price": None}
